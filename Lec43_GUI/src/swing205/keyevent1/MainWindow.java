...
public class MainWindow extends JFrame{
	...
	public MainWindow() {
		...
		for (int i = 0; i < lblMsg.length; i++) {
			...
			// 레이블의 바탕색, 글꼴등  설정 가능
			lblMsg[i].setOpaque(true);  // 바탕색이 보이도록 지정 (불투명0
			lblMsg[i].setBackground(Color.YELLOW);
			lblMsg[i].setFont( new Font("궁서", Font.BOLD, 40));  // java.awt.Font 객체 : (글꼴, style, size)
		} // end for
	...
	class MyKeyListener extends KeyAdapter{
		...
		public void keyPressed(KeyEvent e) {
			...
			int keyCode = e.getKeyCode(); //가상키값 알아내기 (모든키에 대해)
			char keyChar = e.getKeyChar(); // 유니코드(문자) 알아내기 (문자키인 경우만)
			
			lblMsg[0].setText(Integer.toString(keyCode));
			lblMsg[1].setText(Character.toString(keyChar));
			lblMsg[2].setText(KeyEvent.getKeyText(keyCode)); //(가상 키)에 해당하는 키의 이름 문자열 리턴	
