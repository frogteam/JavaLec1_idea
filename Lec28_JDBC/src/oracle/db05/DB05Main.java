...
public class DB05Main implements Query {
	...
	public static void main(String[] args) {
		...
		try {
			...
			conn.setAutoCommit(false);  // 일단
			
			// 2. 쿼리연산(들) 수행
			stmt.executeUpdate("INSERT INTO test_member VALUES (test_member_seq.nextval, 'ddd', SYSDATE)");
			stmt.executeUpdate("INSERT INTO test_member VALUES (test_member_seq.nextval, 'ddd', SYSDATE)");
...
//			}
			...
			// 4. 예외 발생등으로 실패하면 rollback() 실행
			if(conn != null) {
				try {
					System.out.println("롤백 합니다");
					conn.rollback();  // SQLException 잡아주어야 한다
